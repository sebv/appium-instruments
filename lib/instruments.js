...
var Instruments = function (opts) {
  ...
  this.termTimer = null;
  this.killTimer = null;
  this.afterLaunchTimeoutCount = 0;
...
Instruments.prototype.launchHandler = function (err) {
  if (!this.launchHandlerCalledBack) {
    this.afterLaunchTimeoutCount = null;
...
var setAfterSimLaunchTimeout = function  (instance) {
  if (!instance.launchTimeout.afterSimLaunch) return;

  if (!instance._setAfterSimLaunchTimeout) {

    instance._setAfterSimLaunchTimeout = _.debounce(function () {
      if (this.launchTimeout.afterSimLaunch && !_.isNull(this.afterLaunchTimeoutCount) &&
          ++this.afterLaunchTimeoutCount < 50) {
        if (this.socketConnectTimeout) clearTimeout(this.socketConnectTimeout);
        this.socketConnectTimeout = setTimeout(
          this.onSocketNeverConnect('afterLaunch ' + this.afterLaunchTimeoutCount),
          this.launchTimeout.afterSimLaunch);
      }
    }.bind(instance), 250);
  }
  instance._setAfterSimLaunchTimeout();
};

Instruments.prototype.outputStreamHandler = function (output) {
  setAfterSimLaunchTimeout(this);
